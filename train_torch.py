...
from answer_collection_09_09 import filtering_6
import time
...
parser.add_argument('--sentiment',
                    type=str,
                    default='0',
                    help='sentiment for system. 0 is neutral, 1 is negative, 2 is positive.') #텍스트 속의 감성 분류

parser.add_argument('--model_params',
                    type=str,
                    default='2021_09_13/model_chp/model_-last.ckpt',
        ...
        with torch.no_grad():
            while 1:
                ...
                if q == 'quit':
                    break
                if len(q) > 512:
                    print("입력할 수 있는 단어 수가 초과하셨습니다.")
                    continue

                elif len(q) < 4:
                    print("입력할 수 있는 단어 수가 너무 적습니다.")
                    continue
                ...
                while 1:
                    ...
                    input_ids = torch.LongTensor(tok.encode(U_TKN + q + SENT + sent + S_TKN + a)).unsqueeze(dim=0)  #pre-trained - kogpt2가 inputs 값을 추론하는 인덱스
                    #print(U_TKN + ' ' + q + ' ' + SENT + ' ' + sent + ' ' + S_TKN + ' ' + a)
                    #print(tok.encode(U_TKN + q + SENT + sent + S_TKN + a))
                    #print(input_ids)

                    pred = self(input_ids) #fine-tuning kogpt2 loading한 Model에 push


                    gen = tok.convert_ids_to_tokens(# Converts a sequence of ids in wordpiece tokens using the vocab.  vocab 대신 convert_ids_to_tokens 사용
                    ...
                    """
                    if len(torch.max(pred, dim=-1)[0].tolist()[0]) >=1:
        ...
        with torch.no_grad():
            ...
            while 1:
                ...
                if gen == EOS:
                    break
                a += gen.replace('▁', ' ')
            print("Alphado BOT > {}".format(a.strip()))
    """
